...
1. **开发**：处于编码状态，需要**不定时的发布到测试环境中进行测试**
2. **测试**：本版本功能开发完成，此时将代码**完整**的发布到测试环境中，进行内部测试
3. **预发布**：此时APP的后台系统已经部署到**线上环境**中，但是还需要**一部分用户（内部人员，被
邀请的体验用户等）**进行线上测试最新的APP，如果出现问题，能及时的回滚
4. **正式发布**：此时APP已经处于完全推广状态，**通知旧版本的用户进行升级到最新的APP版本可用**，
当然**旧版本还是能保持一段时间的可用状态**，不能马上不可用。
