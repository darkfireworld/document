...
现在互联网开发都处于快速迭代开发的状态。这里我们来讨论一下迭代开发中的**生命周期，代码版本，兼容性设计**等话题。
...
1. **开发阶段**：处于编码状态，源码经过构建（编译，单元测试，打包，发布等）后，**不定时的发布到测试环境中进行集成测试**。
2. **集成测试阶段**：开发功能的版本不断的发布到测试环境中，进行集成测试，直到最后功能全部开发完毕，并且测试通过，然后进入预发布阶段。
3. **预发布阶段**：此时APP的后台系统已经部署到**线上环境**中，但是还需要**一部分用户（内部人员，被邀请的体验用户等）**进行线上测试最新的APP，如果出现问题，能及时的回滚。
4. **正式发布阶段**：此时APP已经处于完全推广状态，**通知旧版本的用户进行升级到最新的APP版本可用**，当然**旧版本还是能保持一段时间的可用状态**，不能马上不可用。
